...
import LeftArrowIcon from "vue-material-design-icons/ArrowLeft.vue";
...
const youtubeUrlWithId = computed(() => {
  if(mealDetails.value.strYoutube) {
    ...
    console.log(index);
    return youtubeUrl.value + mealDetails.value.strYoutube.slice(index+1, mealDetails.value.strYoutube.length);
  }
  return null;
})
</script>
...
<template>
  <MainLayout>
    <div class="w-full flex flex-col gap-1" v-if="!useLoadingStore.isLoading">
      ...
      <div class="flex items-start flex-col md:flex-row gap-6 pb-2">
        <div class="w-full md:w-4/12 flex flex-col">
          <figure class="flex-1 relative detail-img">
            ...
            <div class="absolute inset-0 w-full h-full bg-stone-900 detail-text-overlay z-20">
            </div>
            <div
                class="flex-col flex items-center justify-center absolute inset-0 w-full h-full flex-1 mt-2.5 z-30 detail-text">
              <p class="text-stone-100 text-lg font-bold">{{mealDetails.strTags ? `Tags: ${mealDetails.strTags}` :
                  'No Tags'}}</p>
              <p class="text-stone-100 text-lg font-bold my-2">{{mealDetails.strCategory && `Categories: ${mealDetails
                  .strCategory}`}}</p>
            </div>
          </figure>
      ...
      <Divider/>
      <iframe v-if="youtubeUrlWithId" class="w-full h-[350px] md:h-[500px]"
